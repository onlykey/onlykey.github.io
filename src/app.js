                          ...
                          success: function(data) {
                            ...
                            if (listItem.username) {
                              ...
                              $getpgp_btn.click(async function() {
                                ...
                                var response_text = await p2g_search.getKey(url)
                                $pgp_copybox.val(response_text)
                                $pgp_copybox.show();
                                $pgp_copybox.focus();
                                $pgp_copybox.select();
      ...
      if (onlykeyApi._status) {
        ...
        if (!disable_onlykey) {
          onlykeyApi.initok(() => {
            ...
            var p2g = onlykeyPGP(onlykeyApi, usevirtru);

            if (enable_tokenizer) {
              tokenizer($, async function(itemName, returnValueFN) {
                returnValueFN(await p2g.getKey(itemName));
              });
            }
            ...
            onlykeyApi.request_pgp_pubkey = function() {
              ...
              return new Promise(async function(resolve) {
                ...
                if (pubkey == "" || !pubkey) {
                  ...
                  pubkey = await p2g.getKey(pubkey);
                  resolve({
                    value: pubkey,
                    on_error: error_2
                  });
